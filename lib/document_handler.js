...
DocumentHandler.prototype.handlePost = function(req, res) {
	...
	req.on('data', function(data) {
		...
		if (_this.maxLength && buffer.length > _this.maxLength) {
			...
			res.end(JSON.stringify({message: 'Kod maksimum uzunluğu aşıyor.'}));
		}
	});
	req.on('end', function() {
		if (cancelled) return;
		...
		if (_this.createKey !== '') {
			if (!buffer.startsWith(_this.createKey)) {
				...
				res.writeHead(400, {'content-type': 'application/json'});
				res.end(JSON.stringify({message: 'Yeni kod eklenirken hata oluştu: yanlış anahtar'}));
				return;
			}
	...
	req.on('error', function(error) {
		...
		res.writeHead(500, {'content-type': 'application/json'});
		res.end(JSON.stringify({message: 'Bağlantı hatası.'}));
	});
};
